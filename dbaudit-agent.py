...
#QUEUE_DB    = Path("/var/lib/dbaudit/queue.db")   # cola local SQLite
#LOG_FILE    = Path("/var/log/dbaudit-agent.log")
QUEUE_DB = Path("./data/queue.db")
LOG_FILE = Path("./logs/dbaudit-agent.log")
...
class EventSender:
    def __init__(self, cfg: dict, queue: LocalQueue):
        ...
        self.session.headers.update({
            "Authorization": f"Token {cfg['AGENT_TOKEN']}",
...
def cmd_install_service():
    """Instala el agente como servicio systemd."""
    if os.geteuid() != 0:
