...
import {
  ...
  ChevronRight, TrendingUp, WifiOff
} from "lucide-react";

import { EventService, AgentService, AlertService } from "../services/api";
import { SEVERITY_COLOR, OP_COLOR, STATUS_AGENT, relTime } from "../constants/constants";
// import { fakeEvents, fakeAlerts, fakeAgents, fakeChartData } from "../mocks/mockData";
import {
  Card, StatCard, SeverityBadge, SectionHeader, Btn, Spinner, EmptyState
} from "../components/ui";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setAlerts(Array.isArray(al) ? al : al?.results || []);
        setAgents(Array.isArray(ag) ? ag : ag?.results || []);
      } catch (e) {
        if (cancelled) return;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <SectionHeader
        ...
        subtitle={`Últimas 24h · ${agents.filter(a => a.status === "active").length} BDs activas · ${healthyAg}/${agents.length} agentes online`}
      />
      
      {/* CONTROL GLOBAL loading / error */}
      {loading ? (
